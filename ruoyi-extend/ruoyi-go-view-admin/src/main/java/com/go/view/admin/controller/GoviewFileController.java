 ...
 */
@RestController
@RequestMapping("/goview/file")
@Slf4j
@RequiredArgsConstructor
public class GoviewFileController extends BaseController {
	...
	public AjaxResult remove(String ids){
		Boolean b=iGoviewFileService.removeByIds(StrUtil.split(ids, ',',-1));
		if(b){
			return success();
		}else{
			return error();
	...
	public AjaxResult update(String id,@RequestBody MultipartFile object) throws IllegalStateException, IOException{
		GoviewFile sysFile=iGoviewFileService.getById(id);
		if(sysFile!=null){
	...
	public Object list(long current, long size){
		Page<GoviewFile> page= new Page<GoviewFile>(current, size);
		IPage<GoviewFile> sysFile=iGoviewFileService.page(page, new LambdaQueryWrapper<GoviewFile>());
		return sysFile;
	}
