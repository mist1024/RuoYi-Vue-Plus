...
import com.go.view.admin.common.config.V2Config;
import com.go.view.admin.common.domain.AjaxResult;
import com.go.view.admin.common.domain.ResultTable;
import com.go.view.admin.common.domain.Tablepar;
import com.go.view.admin.common.util.ConvertUtil;
import com.go.view.admin.common.util.SnowflakeIdWorker;
...
import com.go.view.admin.domain.vo.GoviewFileVo;
import com.go.view.admin.domain.vo.GoviewProjectVo;
...
public class GoviewProjectController extends BaseController {
	final IGoviewFileService iGoviewFileService;

	final V2Config v2Config;

	final IGoviewProjectService iGoviewProjectService;
	...
	public AjaxResult upload(@RequestBody MultipartFile object) throws IOException{
		...
		String virtualKey=GoviewFileController.getFirstNotNull(v2Config.getXnljmap());
		String absolutePath=v2Config.getXnljmap().get(GoviewFileController.getFirstNotNull(v2Config.getXnljmap()));
		GoviewFile sysFile=new GoviewFile();
		...
		iGoviewFileService.saveOrUpdate(sysFile);
		File desc = GoviewFileController.getAbsoluteFile(v2Config.getFileurl()+File.separator+filepath,fileSuffixName);
		object.transferTo(desc);
		GoviewFileVo sysFileVo=BeanUtil.copyProperties(sysFile, GoviewFileVo.class);
